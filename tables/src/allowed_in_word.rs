...
/// `[\p{Unassigned}\p{Private_Use}\p{ID_Continue}\p{ID_Compat_Math_Continue}\p{Extended_Pictographic}\p{Emoji_Component}-[\p{Punctuation}-\p{Other_Punctuation}]]`,
...
pub fn allowed_in_word(data: &DataFiles) -> CodepointBitArr {
    ...
    set_by_prop(
        &mut word_component,
        &data.emoji_data,
        "Extended_Pictographic|Emoji_Component",
        true,
    );
...
pub fn allowed_in_word(c: char) -> bool {{
    ...
    let mut late_zeros = allowed_in_word[first_cp_not_in_tree..]
        .iter_zeros()
        .map(|n| n + first_cp_not_in_tree);
    if let Some(first_late_zero) = late_zeros.next() {
        write!(out, "        !matches!(cp, 0x{first_late_zero:06X}")?;
        for late_zero in late_zeros {
            write!(out, "| 0x{late_zero:06X}")?;
        }
        writeln!(out, ")")?;
    } else {
        writeln!(out, "true")?;
